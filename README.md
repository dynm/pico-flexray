...
The channel TX/TXEN GPIOs below are the firmware defaults. You can change the
FR1-FR4 TX and TXEN pins from the web interface before starting transmission.

| GPIO | Signal | Function |
|-----:|--------|----------|
| 2 | BGE | Transceiver bus-guard enable (high = on) |
| 3 | STBN | Transceiver standby (high = active) |
| 28 | TXD_FR1 | Default TX data pin for FR1 transceiver |
| 27 | TXEN_FR1 | Default TX enable pin for FR1 (active low) |
| 4 | TXD_FR2 | Default TX data pin for FR2 transceiver |
| 5 | TXEN_FR2 | Default TX enable pin for FR2 (active low) |
| 10 | TXD_FR3 | Default TX data pin for FR3 transceiver |
| 9 | TXEN_FR3 | Default TX enable pin for FR3 (active low) |
| 16 | TXD_FR4 | Default TX data pin for FR4 transceiver |
| 22 | TXEN_FR4 | Default TX enable pin for FR4 (active low) |
| 20 | LED | Status LED |

Compatible transceivers (active-low TX_EN): TLE9222, TJA1082, NCV7383.

Pin changes are sent to the device when transmission is stopped. GPIOs must be
unique across TX/TXEN assignments, TX and TXEN must differ, and board-control
pins `2`, `3`, `17`, `18`, `19`, and `20` are reserved.
