...
#define CRC8_POLY 0x2F   // SAE J1850 polynomial (MSB-first)

int main() {
    uint8_t crc_table[256];

    printf("Generating CRC-8 poly=0x2F init=0xFF xorout=0xFF Lookup Table...\n");
    ...
    printf("static const uint8_t flexray_crc8_0x2f_table[256] = {\n");
    for (int i = 0; i < 256; ++i) {
        if (i % 8 == 0) {
            printf("    ");
        }
