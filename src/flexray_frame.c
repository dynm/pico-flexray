...
void parse_frame(const uint32_t *raw_buffer, flexray_frame_t *parsed_frame) {
    ...
    parsed_frame->reserved_bit                = get_bits_msb(raw_buffer, current_bit, 1);  current_bit += 1;
    parsed_frame->payload_preamble_indicator  = get_bits_msb(raw_buffer, current_bit, 1);  current_bit += 1;
    parsed_frame->null_frame_indicator        = get_bits_msb(raw_buffer, current_bit, 1);  current_bit += 1;
    parsed_frame->sync_frame_indicator        = get_bits_msb(raw_buffer, current_bit, 1);  current_bit += 1;
    parsed_frame->startup_frame_indicator     = get_bits_msb(raw_buffer, current_bit, 1);  current_bit += 1;
    parsed_frame->frame_id                    = get_bits_msb(raw_buffer, current_bit, 11); current_bit += 11;
    parsed_frame->payload_length_words        = get_bits_msb(raw_buffer, current_bit, 7);  current_bit += 7;
    parsed_frame->header_crc                  = get_bits_msb(raw_buffer, current_bit, 11); current_bit += 11;
    parsed_frame->cycle_count                 = get_bits_msb(raw_buffer, current_bit, 6);  current_bit += 6;
...
bool is_valid_frame(flexray_frame_t *frame, const uint32_t *raw_buffer) {
    ...
    if (frame->frame_id >= TOTAL_FRAMES) return false;
    
    if (frame->payload_length_words > 127) return false;
