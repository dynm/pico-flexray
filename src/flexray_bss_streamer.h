...
#define FR1_RING_SIZE_BYTES   (1u << 15)
#define FR2_RING_SIZE_BYTES   (1u << 15)
#define FR1_RING_MASK         (FR1_RING_SIZE_BYTES - 1)
#define FR2_RING_MASK         (FR2_RING_SIZE_BYTES - 1)

_Static_assert((FR1_RING_SIZE_BYTES & FR1_RING_MASK) == 0,
               "FR1 DMA ring size must be a power of two");
_Static_assert((FR2_RING_SIZE_BYTES & FR2_RING_MASK) == 0,
               "FR2 DMA ring size must be a power of two");
_Static_assert(FR1_RING_SIZE_BYTES <= (1u << 15),
               "notification encoding supports at most a 32 KiB FR1 ring");
_Static_assert(FR2_RING_SIZE_BYTES <= (1u << 15),
               "notification encoding supports at most a 32 KiB FR2 ring");
...
// Encoded format: [31]=is_fr2, [30:29]=fr34_source,
// [28:15]=seq(14 bits), [14:0]=32 KiB ring index.
...
typedef struct {
    ...
    uint32_t seq;        // 14-bit sequence
    uint16_t end_idx;    // 15-bit ring index (end position)
} notify_info_t;
...
static inline void notify_decode(uint32_t encoded, notify_info_t *out)
{
    ...
    out->seq = (encoded >> 15) & 0x3FFF;
    out->end_idx = (uint16_t)(encoded & 0x7FFF);
}
